...
<div class="table" id="table" style="margin: 0;">
  <?php if($total_row !== 0 ) : ?>
    ...
    <div class="pagination">
      <!-- Back -->
       <a onclick="fetchData(<?= $active_page != 1 ? $active_page - 1 : 1 ?>)" class="pagination-direction">
        <i class="fa fa-angle-left"></i> 
      </a>
      <!-- Page Number -->
      <?php for($i = 1; $i <= $total_page; $i++) : ?>
        <a onclick="fetchData(<?= $i ?>)" class="<?= $i == $active_page ? 'pagination-active' : ''?>">
          <?= $i ?>
        </a>
      <?php endfor ?>
      <!-- Next -->
      <a onclick="fetchData(<?= $active_page != $total_page ? $active_page + 1  :  $active_page ?>)" class="pagination-direction">
